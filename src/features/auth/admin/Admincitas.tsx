...
export default function AdminCitas() {
  ...
  const [statusFilter, setStatusFilter] = useState<"Todos" | CitaStatus>("Todos");
  ...
  function handleDraftChange<K extends keyof CitaRecord>(key: K, value: CitaRecord[K]) {
  ...
  function cancelCita() {
    setCitas((prev) =>
      prev.map((c) => (c.id === selected.id ? { ...c, status: "Cancelada" } : c))
    );
  }
    ...
    <>
      ...
      <div className="ipdj-filters">
        ...
        <select
          ...
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value as "Todos" | CitaStatus)}
        >
      ...
      <div className="ipdj-table-wrap">
        <table>
          ...
          <tbody>
              ...
              >
                ...
                <td onClick={(e) => e.stopPropagation()}>
                  <div className="ipdj-actions-cell">
                    <button className="ipdj-act-btn" title="Editar" onClick={() => select(c)}>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} className="cell-muted" style={{ textAlign: "center", padding: 24 }}>
        ...
        <div className="ipdj-table-footer">
          <div className="count">
            Mostrando {Math.min(PAGE_SIZE, filtered.length)} de {citas.length} citas
          </div>
  ...
  const sidePanel = (
    <>
      ...
      <div className="ipdj-field">
        ...
        <select value={displayDj} onChange={(e) => handleDraftChange("dj", e.target.value)}>
          {DJS.map((d) => (
      ...
      <div className="ipdj-field">
        ...
        <select
          value={displayStatus}
          onChange={(e) => handleDraftChange("status", e.target.value as CitaStatus)}
        >
