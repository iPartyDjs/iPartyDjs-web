    ...
    <AdminPageShell
      ...
      sidePanel={
        seleccionada && (
          <PanelDetalleResena
            resena={seleccionada}
            onAprobar={() => cambiarEstado(seleccionada.id, "aprobada")}
            onRechazar={() => cambiarEstado(seleccionada.id, "rechazada")}
...
function TarjetaResena({
  ...
  return (
    <article
      className={"ipdj-resena-card" + (seleccionada ? " seleccionada" : "")}
      onClick={onClick}
    >
      <header className="ipdj-resena-card-header">
        ...
        <div>
          ...
          <span className="ipdj-resena-tipo">{resena.tipoEvento}</span>
        </div>
