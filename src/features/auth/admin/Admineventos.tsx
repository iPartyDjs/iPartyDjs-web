...
export default function AdminEventos() {
    ...
    const [errorSolicitudes, setErrorSolicitudes] = useState<string | null>(
        null,
    );
    ...
    useEffect(() => {
        ...
        function extractEventItems(result: unknown): EventoDTO[] {
            ...
            if (result && typeof result === "object") {
                ...
                const candidate =
                    obj.items ?? obj.data ?? obj.results ?? obj.eventos;
    ...
    useEffect(() => {
        ...
        function extractItems(result: unknown): SolicitudEventoDTO[] {
            ...
            if (result && typeof result === "object") {
                ...
                if (Array.isArray(candidate))
                    return candidate as SolicitudEventoDTO[];
            }
            return [];
        }
        ...
        async function fetchSolicitudes() {
            try {
                ...
                const result = await solicitudService.listAll(
                    "completada",
                    1,
                    100,
                );
    ...
    const { data: resenia, isLoading: loadingResenia } = useReseniaByEvento(
        selected?.id_evento ?? "",
    );
    const aprobarResenia = useAprobarResenia();
    const rechazarResenia = useRechazarResenia();
    const eliminarResenia = useEliminarResenia();

    function handleAprobarResenia() {
        if (!resenia) return;
        aprobarResenia.mutate(resenia.id_resenia, {
            onError: (err) => {
                const message =
    ...
    function handleRechazarResenia() {
        if (!resenia) return;
        const motivo =
            window.prompt("Motivo del rechazo (opcional):") ?? undefined;
        rechazarResenia.mutate(
    ...
    function handleEliminarResenia() {
        if (!resenia) return;
        if (
            !window.confirm(
            ...
            return;
        eliminarResenia.mutate(resenia.id_resenia, {
            onError: (err) => {
                const message =
                    err instanceof Error
                        ? err.message
    ...
    const stats = useMemo(() => {
        ...
        const realizados = events.filter(
    ...
    async function saveChanges() {
        ...
        const payload: {
            fecha_hora?: Date;
            direccion?: string;
            tipo_evento?: EventoDTO["tipo_evento"];
        } = {};

        if (draft.fecha_hora !== undefined && draft.fecha_hora !== null) {
            const fechaValue = draft.fecha_hora as unknown;
            payload.fecha_hora =
                fechaValue instanceof Date
                    ? fechaValue
                    : new Date(String(fechaValue));
        }
        if (draft.direccion !== undefined) {
            payload.direccion = String(draft.direccion);
        }
        if (draft.tipo_evento !== undefined) {
            payload.tipo_evento = draft.tipo_evento as EventoDTO["tipo_evento"];
        }

        if (Object.keys(payload).length === 0) return;

        try {
            const updated = await eventoService.update(id, payload);
            setEvents((prev) =>
                prev.map((e) =>
                    e.id_evento === id ? { ...e, ...updated } : e,
                ),
            );
            setDraft({});
    ...
    function selectSolicitud(s: SolicitudEventoDTO) {
        setNewEvent((n) => ({
            ...
            tipo_evento:
                (s.tipo_evento as EventoDTO["tipo_evento"]) ?? n.tipo_evento,
        }));
    }
    ...
    const mainContent = (
        <>
            ...
            {loading ? (
                ...
                <div className="ipdj-event-grid">
                    ...
                    foto, así que solo mostramos badges + datos reales. */}
                                ...
                                >
                                    <span className="ipdj-event-category">
                                        {categoryName}
                                    </span>
                                    <span
            ...
            <div className="ipdj-table-footer">
                <div className="count">
                    Mostrando {Math.min(PAGE_SIZE, filtered.length)} de{" "}
        ...
        <>
            ...
            <div className="ipdj-field">
                ...
                <input
                    ...
                    onChange={(e) =>
                        handleDraftChange("fecha_hora", e.target.value)
                    }
                />
            </div>
            <div className="ipdj-field">
                ...
                <input
                    ...
                    onChange={(e) =>
                        handleDraftChange("direccion", e.target.value)
                    }
                />
            </div>
            <div className="ipdj-field">
            ...
            <div className="ipdj-field">
                ...
                <select
                    ...
                    onChange={(e) =>
                        handleDraftChange(
                            "estado",
                            e.target.value as EventoDTO["estado"],
                        )
                    }
                >
            ...
            {selected.estado === "realizado" && (
                ...
                >
                    ...
                    {loadingResenia && (
                        <div className="cell-muted">Cargando reseña...</div>
                    )}

                    {!loadingResenia && !resenia && (
                        <div className="cell-muted">
                            Este evento aún no tiene reseña.
                        </div>
                    )}
                        ...
                        <>
                            ...
                            {resenia.estado === "pendiente" && (
                                <div className="ipdj-actions-cell">
                                    ...
                                    <button
                                        ...
                                        disabled={rechazarResenia.isPending}
                                        onClick={handleRechazarResenia}
                                    >
                                        Rechazar
                                    </button>
                                </div>
                            ...
                            {resenia.estado === "aprobado" &&
                                !reseniaEdicionPendiente && (
                                    <button
                                        className="ipdj-btn-danger"
                                        disabled={eliminarResenia.isPending}
                ...
                <div className="ipdj-field">
                    ...
                    {!loadingSolicitudes &&
                        ...
                        solicitudesDisponibles.length === 0 && (
                            <div
                                ...
                                style={{ padding: "0.5rem 0" }}
                            >
                                No hay solicitudes completadas pendientes de
                                convertirse en evento.
                            </div>
                        )}
                    ...
                    {!loadingSolicitudes &&
                        ...
                        !newEvent.id_solicitud && (
                            ...
                            >
                                ↑ Haz clic en una fila de la tabla para
                                seleccionar la solicitud antes de crear el
                                evento.
                            </div>
                        )}
                </div>
                ...
                <div className="ipdj-field">
                    ...
                    <select
                        ...
                        onChange={(e) =>
                            setNewEvent((n) => ({
                                ...n,
                                tipo_evento: e.target
                                    .value as EventoDTO["tipo_evento"],
                            }))
                        }
                    >
                        {CATEGORIES.map((c) => (
